...
void SimpleGnuplot::setXLabel(std::string labelName)
{
    _myGp << "set xl " << labelName << std::endl;
}

void SimpleGnuplot::setYLabel(std::string labelName)
{
    _myGp << "set yl " << labelName << std::endl;
}
...
{
    if (isFirst)
        _myGp << "plot '-' tit " << name << " "<< lineType << lineColor << color << withLine << with;
    else 
...
{
    //也就是说学习造型拉满了我小无敌，这个比学习天赋起飞了，只要疯狂拆房子就能升建筑等级。
    _myGp << std::endl;
